<template>
    <canvas :class="canvasClass" ref="canvas"></canvas>
</template>
...
const emit = defineEmits<{
    (e: 'onaction', action: string): void
}>()
// ref组件
const canvas = ref<HTMLCanvasElement>();
const fClass = reactive(props.modelValue)
...
Model.onAction = (action: string) => {
    switch (action) {
        default: fClass.pause = true; break;
        case 'walk': fClass.pause = false; break;
    }
}
